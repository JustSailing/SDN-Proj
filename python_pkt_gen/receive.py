...
def handle_pkt(pkt, iface, _fil):
    ...
    if(ty == 2):
        ...
        for x in range(st, end + 1):
            memLck.acquire()
            sharedMem[x] = dat[index]
            memLck.release()
            index = index + 1
        finish = datetime.now()
        duration = finish - now
        comp_pkt = Ether(src=get_if_hwaddr(iface), dst=pkt[Ether].src, type=TYPE_IPV4) /  Mess(prot=TYPE_IPV4, ms = 5, id = pkt[Mess].id, start = 0 , end = 0) / IP (dst = pkt[IP].src) / UDP(dport=4321, sport=1234) / Raw(format(f"{duration.microseconds}"))
        ...
        for x in range(st, end + 1):
            if(index == 1000):
                index = 0
                p = Ether(src=get_if_hwaddr(iface), dst=pkt[Ether].src, type=TYPE_IPV4) /  Mess(prot=TYPE_IPV4, ms = 1, id = pkt[Mess].id, start = prev_star , end = star) / IP (dst = pkt[IP].src) / UDP(dport=4321, sport=1234) / Raw(sending) 
                sendp(p, iface)
                sending = ""
                prev_star = star
                continue
            ...
            star += 1
        if(sending != ""):
            p = Ether(src=get_if_hwaddr(iface), dst=pkt[Ether].src, type=TYPE_IPV4) /  Mess(prot=TYPE_IPV4, ms = 1, id = pkt[Mess].id, start = prev_star , end = end) / IP (dst = pkt[IP].src) / UDP(dport=4321, sport=1234) / Raw(sending) 
            sendp(p, iface)
        finish = datetime.now()
        duration = finish - now
        comp_pkt = Ether(src=get_if_hwaddr(iface), dst=pkt[Ether].src, type=TYPE_IPV4) /  Mess(prot=TYPE_IPV4, ms = 5, id = pkt[Mess].id, start = 0 , end = 0 ) / IP (dst = pkt[IP].src) / UDP(dport=4321, sport=1234) / Raw(format(f"{duration.microseconds}"))
...
def receive(iface, _fil):
    bind_layers(Ether, Mess, type=TYPE_IPV4)
