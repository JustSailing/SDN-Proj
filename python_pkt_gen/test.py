    ...
    if (pkt[Mess].msg_id == WRITE):
        if (pkt[Mess].type == ADD):
            ...
            if (node == None):
                ...
                pkt[Ether].dst = temp
                pkt[IP].dst = pkt[IP].src
                pkt[Mess].msg_id = COMPLETED
                pkt[Mess].time = time.time()
                pkt[Raw].load = b""
                sendp(pkt)
                pkt_copy[UDP].payload.load = node.txt.decode('ascii')
        ...
        if (pkt[Mess].type == ADD_NODE):
            ...
            if (node == None):
                ...
                node.lock.acquire()
                node.next = new_node.uuid
                node.lock.release()
                if(temp != None):
                    new_node.next = temp.uuid
                    temp.lock.acquire()
                    temp.prev = new_node.uuid
                    temp.lock.release()
                map_lock.acquire()
...
def handle_update(pkt):
    ...
    p = Ether(src=pkt[Ether].dst, dst=pkt[Ether].src, type=TYPE_MESS) / Mess(prot=TYPE_IPV4, msg_id=UPDATE, uuid=node.uuid,  len_txt=node.len_txt, txt_blk_idx=node.txt_blk_idx,
                                                                                 time=node.time, start_idx=node.start_idx, end_idx=node.end_idx, prev=node.prev, next=node.next, beg_node = 1) / IP(dst=pkt[IP].src, src=pkt[IP].dst) / UDP(dport=4321, sport=1234) / Raw(node.txt.decode('ascii'))
    sendp(p)
    w = open(log_file, "a")
    # index = 1
    map_lock.acquire()
    node = map_text_node.get(node.next, None)
    map_lock.release()
    # node = node.next
    while (node != None):
        node.lock.acquire()
        p = Ether(src=pkt[Ether].dst, dst=pkt[Ether].src, type=TYPE_MESS) / Mess(prot=TYPE_IPV4, msg_id=UPDATE, uuid=node.uuid,  len_txt=node.len_txt, txt_blk_idx=node.txt_blk_idx,
                                                                                 time=node.time, start_idx=node.start_idx, end_idx=node.end_idx, prev=node.prev, next=node.next, beg_node = 0) / IP(dst=pkt[IP].src, src=pkt[IP].dst) / UDP(dport=4321, sport=1234) / Raw(node.txt.decode('ascii'))
        sendp(p)
